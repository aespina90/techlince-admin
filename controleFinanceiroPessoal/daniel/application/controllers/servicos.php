...
class Servicos extends CI_Controller {
	...
	function gerenciar(){
		...
		$this->data['results'] = $this->servicos_model->get('servicos','idServicos,nome,descricao,preco,clienteResponsavel,diaJogo,inicioPartida,fimPartida,quadra','',$config['per_page'],$this->uri->segment(3));
    ...
    function adicionar() {
        ...
        if ($this->form_validation->run('servicos') == false) {
            ...
            $data = array(
                ...
                'diaJogo' => set_value('diaJogo'),
                'inicioPartida' => set_value('inicioPartida'),
                'fimPartida' => set_value('fimPartida'),
                'quadra' => set_value('quadra'),
                'preco' => $preco
            );

            if ($this->servicos_model->add('servicos', $data) == TRUE) {
                $this->session->set_flashdata('success', 'Equipe adicionada com sucesso!');
    ...
    function editar() {
        ...
        if ($this->form_validation->run('servicos') == false) {
            ...
            $data = array(
                ...
                'clienteResponsavel' => $this->input->post('clienteResponsavel'),
                'diaJogo' => $this->input->post('diaJogo'),
                'inicioPartida' => $this->input->post('inicioPartida'),
                'fimPartida' => $this->input->post('fimPartida'),
                'quadra' => $this->input->post('quadra'),
                'preco' => $preco
            );

            if ($this->servicos_model->edit('servicos', $data, 'idServicos', $this->input->post('idServicos')) == TRUE) {
                $this->session->set_flashdata('success', 'Equipe editada com sucesso!');
